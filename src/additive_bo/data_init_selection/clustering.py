...
class BOInitDataSelection:
    ...
    def fit(self, x, exclude: list = None):
        ...
        if self.init_method == "believer":
            ...
            print(ix, '<- the real next selected point', 'the distance to that point', distances[prev_selected_point, ix])

            distances = np.delete(distances, prev_selected_point, axis=0)
            distances = np.delete(distances, prev_selected_point, axis=1)
            
            ix = np.argmax(distances[prev_selected_point, :])
            print(ix, '<- the fake next selected point', 'the distance to the fake point', distances[prev_selected_point, ix])
            ...
            print(prev_selected_point, "initial selected point")
            init_indices_from_clusters = [prev_selected_point]
            closest_indices = np.argsort(distances[prev_selected_point, :])[1:self.n_clusters]
            ...
            for center in cluster_centers:
                ...
                closest_indices = np.argsort(distances.flatten())[:num_points//len(cluster_centers)]
                init_indices_from_clusters.extend(closest_indices)
            ...
            for i in range(self.n_clusters - 1):
                distances = cdist(x_init, x_init[init_indices_from_clusters], metric=self.metric)
            ...
            bounds = [(torch.min(x_init[:, i]), torch.max(x_init[:, i])) for i in range(x_init.shape[-1])]
            ...
            init_indices_from_clusters = np.apply_along_axis(lambda x: np.argmin(cdist(x_init, np.array([x]), metric=self.metric)), 1, input_points)
            ...
            input_points = lhs(len(bounds), samples=self.n_clusters, criterion='maximin')
            # scale to bounds
            input_points = input_points * (np.array(bounds)[:, 1] - np.array(bounds)[:, 0]) + np.array(bounds)[:, 0]
            ...
            init_indices_from_clusters = np.apply_along_axis(lambda x: np.argmin(cdist(x_init, np.array([x]), metric='euclidean')), 1, input_points)
            init_indices_from_clusters = init_indices_from_clusters.tolist()
            ...
            init_indices_from_clusters = np.argsort(np.sum(pc_scores**2, axis=1))[-self.n_clusters:]
            init_indices_from_clusters = init_indices_from_clusters.tolist()
